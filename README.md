...
2. 자잘한 애니메이션 수정
> 플레이어 움직이는 애니메이션 수정
3. 스테이지 추가 및 몬스터 체력 증가
> 몬스터를 10마리 잡았을 때, 스테이지가 더 어려운 스테이지로 변경 시 몬스터의 체력이 추가로 증가
4. 효과 추가
> 포스트 프로세싱 비네트 효과 추가
5. 플레이어 사망 이벤트 추가
> 플레이어 사망 시 플레이어 사망 애니메이션 재생 및 페이드 인, 아웃 이전 스테이지로 갈 것인지, 재도전 할 것인지 버튼 출력
> 재도전 시 플레이어 체력이 Max체력으로 변경 된 후 다시 플레이
> 증가 된 몬스터의 체력 초기화 플레이어는 그대로
